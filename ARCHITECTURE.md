...
### Game
Содержит текущий уровень, а также `ModelCharacterConnection`, позволяющий управлять
персонажем, которого контролирует игрок или `NPCController`)
...
### Behaviour
Задает поведение, в соответствии с которым `NPCController` управляет
персонажем.

### GameMap
Игровая карта, состоящая из `width` на `height` клеток. В каждой клетке
...
для определения картинки, с помощью которой данный объект нужно отображать.
...
### ImmutableCharacter
Неизменяемый персонаж, который только предоставляет доступ к параметрам,
присущим всем персонажам (здоровье, инвентарь и т.п.).
