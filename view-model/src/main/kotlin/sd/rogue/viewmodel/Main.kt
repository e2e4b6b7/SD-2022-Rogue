...
data class GameConfig(val enemiesCount: Int, val width: Int, val height: Int)

fun init(config: GameConfig): Module {
    val game = Game(LevelGenerator.generateRandomLevel(config.enemiesCount, config.width, config.height))
    return module {
