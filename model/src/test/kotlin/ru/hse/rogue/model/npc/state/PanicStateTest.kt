...
class PanicStateTest {
    ...
    private val startHealth = 2u * State.DEFAULT_HEALTH_THRESH_HOLD
    ...
    @Test
    fun `check behaviour change after health decrease and nothing happened`() {
        gameMap[0, 0] = char
    ...
    @Test
    fun `check behaviour change after health decrease and passive regeneration`() {
        gameMap[0, 0] = char
        assertEquals(mob.behaviour, startBehaviour)
        mob.character.healthDecrease(startHealth - (State.DEFAULT_HEALTH_THRESH_HOLD - 1u))
        mob.state.changeState(mob, playerChar)
        ...
        mob.character.passiveRegeneration()
        repeat(10) {
            mob.state.changeState(mob, playerChar)
            mob.behaviour.doAnything(modelConnection)
            assertEquals(mob.behaviour, startBehaviour)
